    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for category, texts in test_cases.items():
                ...
                for text in texts:
                    sentiment = sentiment_labels[index] if sentiment_labels else \
                               ("Positive" if predictions[index] in [1, 4] else "Negative")
                    
                    expected_positive = category == "positive"
                    is_positive = sentiment == "Positive"
                    is_correct = (expected_positive == is_positive) if category in ["positive", "negative"] else None
                ...
                if category in ["positive", "negative"]:
                    ...
                    print(f"{Fore.CYAN}Accuracy for {category}: {accuracy:.1f}%{Style.RESET_ALL}")
            
            if accuracy_by_category:
                total_correct = sum(accuracy_by_category[cat] * len(test_cases[cat]) for cat in accuracy_by_category)
                total_samples = sum(len(test_cases[cat]) for cat in accuracy_by_category)
                overall_accuracy = total_correct / total_samples
                print(f"\n{Fore.GREEN}Overall accuracy: {overall_accuracy:.1f}%{Style.RESET_ALL}")
        ...
        print(f"{Fore.RED}✗ Connection error: Could not connect to the API at {url}{Style.RESET_ALL}")
        print("Make sure the Flask server is running and the URL is correct.")
    except Exception as e:
        print(f"{Fore.RED}✗ Error: {str(e)}{Style.RESET_ALL}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test the SentiVista sentiment analysis API')
    parser.add_argument('--url', default="http://127.0.0.1:5000/predict", help='API endpoint URL')
    parser.add_argument('--model', default="sentiment_model_lr.pkl", help='Model to use (e.g., nb for Naive Bayes, lr for Logistic Regression)')
